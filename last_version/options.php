...
?>
    <form method="post"
          action="<?php echo $APPLICATION->GetCurPage(); ?>?mid=<?php echo htmlspecialcharsbx($mid); ?>&lang=<?php echo LANGUAGE_ID; ?>">
        <?php
...
$tabControl->Buttons(); ?>
        <script>
            function RestoreDefaults() {
                if (confirm('<?php echo addslashes(Loc::getMessage('MAIN_HINT_RESTORE_DEFAULTS_WARNING')); ?>'))
                    ...
                    ); ?>?restore=Y&lang=<?php echo LANGUAGE_ID; ?>&mid=<?php echo urlencode($mid); ?>&<?php echo bitrix_sessid_get(); ?>";
            }
        </script>
        <input <?php
        if ($rights < 'W') {
            echo 'disabled';
        } ?> type="submit" name="Update" value="<?php echo Loc::getMessage('MAIN_SAVE'); ?>">
        ...
        if ($rights < 'W') {
            echo 'disabled';
             ...
             value="<?php echo Loc::getMessage('VASOFT_CORE_DEFAULTS'); ?>">
    </form>
<?php
$tabControl->End();
